...
public class ListViewItem extends LinearLayout implements View.OnClickListener {
    ...
    private static String PACKAGE_PREFIX = "com.gcatech.ipat";
    ...
    public void onClick(View v) {
        ...
        SetViewWrapper viewGrapper = null;

        if (!isBasicFunctionListItem) {
            viewGrapper = new SetViewWrapper(mChildrenLayoutId, mTitleId, getContext());
            viewGrapper.viewName = mViewNameString;

            EventBus.getDefault().post(viewGrapper);

        } else {
            try {
                viewGrapper = new SetViewWrapper(getView(), mTitleId);
                viewGrapper.viewName = mViewNameString;
                EventBus.getDefault().post(viewGrapper);
